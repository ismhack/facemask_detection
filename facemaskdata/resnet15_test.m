...
resnet = trainNetwork(augmentedTrainingSet,resnet15_layers,options);


%% test

Ypred = classify(resnet, augmentedTestSet); % Predict labels of test data.
...
tic; [result boxes scores] = cnn_detector_demo(test, .6, resnet, 1); toc;
...
tic; y =resnet.predict(window1); toc;
y = classify(resnet, window1)
figure(2); 
subplot(1,3,1);
imshow((window1),[]);
title(strcat(' pred:', string(y), ' true:1' ));

y =resnet.predict(window2)
y = classify(resnet, window2);
subplot(1,3,2);
imshow((window2),[]);
title(strcat(' pred:', string(y), ' true:1' ));

tic; y =resnet.predict(window3); toc;
y = classify(resnet, window3);
subplot(1,3,3);
imshow((window3),[]);
title(strcat(' pred:', string(y), ' true:1' ));
...
tic; result1 = cnn_skin_demo(test,[.2 .3 .4],resnet, positives, negatives, 5); toc;
...
tic; result1 = cnn_skin_demo(test,[0.4 .5 .6 ],resnet, positives, negatives, 1); toc;
figure(5); imshow(result1, []);
