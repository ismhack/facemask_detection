...
vggnet = trainNetwork(augmentedTrainingSet,vgg19_layers,options);


%% test

Ypred = classify(vggnet, augmentedTestSet); % Predict labels of test data.
...
%tic; [result boxes scores] = cnn_detector_demo(test, 1, vggnet, 1); toc;
...
tic; y =vggnet.predict(window1); toc;
...
y =vggnet.predict(window2)
y = classify(net, window2);
subplot(1,3,2);
imshow((window2),[]);
title(strcat(' pred:', string(y), ' true:1' ));

y =vggnet.predict(window3)
...
tic; result1 = cnn_skin_demo(test,[.45 0.5 .8],vggnet, positives, negatives, 5); toc;
...
tic; result1 = cnn_skin_demo(test,[0.5 .8 1],vggnet, positives, negatives, 5); toc;
figure(5); imshow(result1, []);
