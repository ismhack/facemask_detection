...
%% performance test 
disp('cnn performance on single patch');
...
disp('cnn with skin detection');
test = (imread('data/eval_cnn_photos/photos/test2.jpg'));
positives = read_double_image('data/positives.bin');
negatives = read_double_image('data/negatives.bin');
tic; [result1, boxes, temp_result, max_scales] = cnn_skin_demo(test,[1],net, positives, negatives, 1); toc;
figure(7); imshow(result1, []);
