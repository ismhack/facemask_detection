...
if(size(positives,1)> 0 )
    ...
    %figure(7); imshow(skin_detection, []);
...
for vertical = 1:(vertical_size-face_vertical)
    for horizontal = 1:3:(horizontal_size-face_horizontal)
        ...
        if(size(positives,1)> 0 )
            ...
            if percentage_skin_pixels < 0.1
                result(row, col) = -1;
                continue;
            end
            %figure(7); imshow(image(vertical:vertical + face_vertical, horizontal:horizontal + face_horizontal) , []);
...
%fprintf(' row: %d, col: %d', vertical + floor(face_vertical/2), horizontal +floor(face_horizontal/2));
...
%fprintf(' A:%d, B:%d, C:%d, D:%d, total: %d,  perc: %f ', A, B, C, D, (A+D - C -B), result);
end
